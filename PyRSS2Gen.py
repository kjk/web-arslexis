...
def _format_date(dt):
    """convert a time into an RFC 822 formatted date
    ...
    return strftime("%a, %d %b %Y %H:%M:%S GMT", dt)
...
class DateElement:
    """implements the 'publish' API for a time

    Takes the tag name and the time to publish.

    Converts the time to RFC 2822 timestamp (4-digit year).
    """
...
class RSS2(WriteXmlMixin):
    ...
    def __init__(self,
                 ...
                 managingEditor = None,
                 webMaster = None,
                 pubDate = None,  # a time, *in* *GMT*
                 lastBuildDate = None, # a time
